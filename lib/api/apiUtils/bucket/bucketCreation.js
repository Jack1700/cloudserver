...
function createBucket(authInfo, bucketName, headers,
    ...
    const locationConstraintParse =
        locationConstraint.split(constants.ingestionDelimiter);
    if (locationConstraintParse[0] !== undefined) {
        bucket.setLocationConstraint(locationConstraintParse[0]);
    }
    if (locationConstraintParse[1] !== undefined) {
        bucket.setIsIngestion(locationConstraintParse[1]);
    }
    ...
    (err, results) => {
        ...
        if (isLegacyAWSBehavior(locationConstraintParse[0]) &&

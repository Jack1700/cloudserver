...
function createBucket(authInfo, bucketName, headers,
    ...
    const { locationConstraintStr, isIngestion } =
        locationConstraint.split(zenkoSeparator);
    if (locationConstraintStr !== undefined) {
        bucket.setLocationConstraint(locationConstraintStr);
    }
    if (isIngestion !== undefined) {
        bucket.setIsIngestion(isIngestion);
    }
    ...
    (err, results) => {
        ...
        if (isLegacyAWSBehavior(locationConstraintStr) &&
